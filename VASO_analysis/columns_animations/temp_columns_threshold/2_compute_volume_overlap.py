...
x = np.mean(consistency, axis=0)
y = scipy.stats.sem(consistency, axis=0)

fig, axs = plt.subplots(dpi=my_dpi)
axs.errorbar(threshold, x, y, color='black')
axs.set_xlabel("Columnarity index")
axs.set_ylabel("% Consistent voxels")
plt.grid()
plt.savefig(os.path.join(PATH_OUT, "Summary_consistent_columns_sem.png"), bbox_inches='tight', dpi=my_dpi)
plt.savefig(os.path.join(PATH_OUT, "Summary_consistent_columns_sem.svg"), bbox_inches='tight', dpi=my_dpi)
