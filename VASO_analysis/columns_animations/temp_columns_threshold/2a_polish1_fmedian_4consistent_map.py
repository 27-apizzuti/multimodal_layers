...
SUBJ = ['sub-02']
FUNC = ['consistent']
threshold = np.arange(64, 100, 1)

for itsbj, su in enumerate(SUBJ):
    PATH_OUT = os.path.join(STUDY_PATH, 'Results', 'Animations', su, 'data', 'consistent_columns', 'polished_columns')
    if not os.path.exists(PATH_OUT):
        os.makedirs(PATH_OUT)
    for fu in FUNC:
        for  itThr in threshold:
            FILE = os.path.join(STUDY_PATH, 'Results', 'Animations', su, 'data', 'consistent_columns', '{}_leftMT_Sphere16radius_{}_winner_map_{}_COL_MASK.nii.gz'.format(su, fu, itThr))
