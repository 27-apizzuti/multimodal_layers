...
Chop the columns: polish with mode filter (remove 45% noise in columns)
...
for itsbj, su in enumerate(SUBJ):
    for fu in FUNC:
        PATH_IN = os.path.join(STUDY_PATH, 'Results', 'Animations', su, 'data', 'columns_threshold', 'polished_columns')
        PATH_OUT = os.path.join(STUDY_PATH, 'Results', 'Animations', su, 'data', 'columns_threshold', 'polished_columns_2')
