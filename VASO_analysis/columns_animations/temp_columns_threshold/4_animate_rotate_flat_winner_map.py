...
for itsbj, su in enumerate(SUBJ):
    ...
    PATH_IN = os.path.join(STUDY_PATH, 'Results', 'Animations', su, 'data', 'columns_threshold', 'polished_columns_2', 'ready-for-animation')
    for fu in FUNC:
        for it, Thr in enumerate(threshold):

            FILENAME = os.path.join(PATH_IN, '{}_{}_winner_map_{}_COL_MASK_plus_curvature_middle_depth_to_the_bottom.nii.gz'.format(su, fu, Thr))
            ...
            for it_class in CLASS:
                ...
                p.camera.zoom(1.3)
                # Move camera
                # p.camera.elevation -= 45 / NR_FRAMES * it
