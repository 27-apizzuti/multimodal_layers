#!/bin/bash
# L-O-R-O CV series
# This script performs BOLD-correction on Training Set (single run).


# # TR-P02:2.4112 / P03: 2.4103 / P04: 2.4771
# Run P04: REF.[/vaso_analysis/standard] & [vaso_analysis/magn_only_noNOISE] 
# Run P03: NORDIC[vaso_analysis/magn_only_noNOISE] & REF.[/vaso_analysis/standard] --> 3 runs
# Run P02: NORDIC[vaso_analysis/magn_only] --- REF.[/vaso_analysis/standard] ---> 4 runs
...
mywork=/mnt/d/Pilot_Exp_VASO/pilotAOM/${SUBJ}/derivatives/func/AOM/vaso_analysis/magn_only_noNOISE/cross_validation
myscript=/mnt/d/Pilot_Exp_VASO/AOM-project

### Part 1. BOLD correction for single run
echo "No averaging across runs: working at single run"

for myrun in ${cases[@]}
do
	echo "Working on run " ${myrun}
	mywork_singleRun=${mywork}/run_${myrun}/boco
	...
	nulled_sr=Nulled_Basis_$(($myrun-1))b.nii
	not_nulled_sr=Not_Nulled_Basis_$(($myrun-1))a.nii
