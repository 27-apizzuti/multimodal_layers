...
# Prepare plot
my_dpi = 96
fig, axs = plt.subplots(nrows=2, ncols=len(SUBJ), figsize=(1920/my_dpi, 1080/my_dpi), subplot_kw=dict(box_aspect=1), dpi=my_dpi)
n_bins = 50
...
for itSubj, su in enumerate(SUBJ):
    ...
    # Loading benchmark dataset (random)
    ...
    for itFu, fu in enumerate(FUNC):
        ...
        if fu == 'BOLD':
            # // Empirical data
            axs[itFu, itSubj].plot(eval_points, y_emp, color = "blue")
            # n, x, _ = axs[itFu, itSubj].hist(temp_data, bins=n_bins, density=True)
        else:
            axs[itFu, itSubj].plot(eval_points, y_emp, color = "red")
            
        # //Random data
        axs[itFu, itSubj].plot(eval_points, y_rand, color = "black")
        # n, x, _ = axs[itFu, itSubj].hist(temp_datar, bins=n_bins, density=True, color = "skyblue")
        # ------------------------------------------------------
        # axs[itFu, itSubj].plot(x, density_ran(x), 'r')
        # axs[itFu, itSubj].plot(x, density_emp(x), 'b')
        # axs[itFu, itSubj].vlines(np.mean(temp_datar), 0, 5)
        axs[itFu, itSubj].set_xlim([0, 1])
        axs[itFu, itSubj].set_xlabel("Columnarity index")
        axs[itFu, itSubj].set_title("Sub-0{}".format(itSubj+1))
