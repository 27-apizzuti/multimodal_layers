...
# -------------------------------------------------------------
    ...
    # Group result
    my_dpi = 96
    fig, axs = plt.subplots(nrows=1, ncols=1, figsize=(1920/my_dpi, 1080/my_dpi), subplot_kw=dict(box_aspect=1), dpi=my_dpi)
    
    axs.errorbar(col_range, sp_map_col[isu, :, 0], err_sp_map_col[isu, :, 0], color='blue')
    axs.errorbar(col_range, sp_map_col[isu, :, 1], err_sp_map_col[isu, :, 1], color='red')
    axs.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
    axs.set_xlabel("Columnarity index")
    axs.set_ylabel("Specificity")
    axs.grid()
    fig_filename = "{}_Specificity_BOLD_VASO_Columnarity_sem".format(su)
    ...
    axs.errorbar(col_range, sen_map_col[isu, :, 0], err_sen_map_col[isu, :, 0], color='blue')
    axs.errorbar(col_range, sen_map_col[isu, :, 1], err_sen_map_col[isu, :, 1], color='red')
    axs.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
    axs.set_xlabel("Columnarity index")
    axs.set_ylabel("Sensitivity")
    axs.grid()
    
    fig_filename = "{}_Senitivity_BOLD_VASO_Columnarity_sem".format(su)
    plt.savefig(os.path.join(PATH_OUT, '{}.jpeg'.format(fig_filename)), bbox_inches='tight', dpi=my_dpi)
    plt.savefig(os.path.join(PATH_OUT, '{}.svg'.format(fig_filename)), bbox_inches='tight', dpi=my_dpi)
    plt.show()
