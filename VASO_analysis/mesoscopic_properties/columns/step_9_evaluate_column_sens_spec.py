...
Group plot (Figure 8A): mean + standard error across participants
...
axs.errorbar(col_range*100, np.mean(sp_map_col[:, :, 0], axis=0), scipy.stats.sem(sp_map_col[:, :, 0], axis=0), color='blue')
axs.errorbar(col_range*100, np.mean(sp_map_col[:, :, 1], axis=0), scipy.stats.sem(sp_map_col[:, :, 1], axis=0), color='red')
axs.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
axs.set_xlabel("Columnarity index")
axs.set_ylabel("Specificity")
axs.grid()
fig_filename = "GroupSbj_Specificity_BOLD_VASO_Columnarity_sem"
...
my_dpi = 200
fig, axs = plt.subplots(nrows=1, ncols=1)
axs.errorbar(col_range*100, np.mean(sen_map_col[:, :, 0], axis=0), scipy.stats.sem(sen_map_col[:, :, 0], axis=0), color='blue')
axs.errorbar(col_range*100, np.mean(sen_map_col[:, :, 1], axis=0), scipy.stats.sem(sen_map_col[:, :, 1], axis=0), color='red')
axs.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
axs.set_xlabel("Columnarity index")
axs.set_ylabel("Sensitivity")
axs.grid()

fig_filename = "GroupSbj_Senitivity_BOLD_VASO_Columnarity_sem"
plt.savefig(os.path.join(PATH_OUT, '{}.jpeg'.format(fig_filename)), bbox_inches='tight', dpi=my_dpi)
plt.savefig(os.path.join(PATH_OUT, '{}.svg'.format(fig_filename)), bbox_inches='tight', dpi=my_dpi)
plt.show()
