...
CONDT = ['standard']
FUNC = ["BOLD_vox", "VASO_vox", "COMM_vox", "CV_BOLD", "CV_VASO", "BOLD_FDR"]  # name of the masks will be created
ROI_NAME = ['rightMT_Sphere16radius']

for roi in ROI_NAME:

    # Loading groups indices
    PATH_IN = os.path.join(STUDY_PATH, 'Results', 'Cross_Validation')
    ...
    for iterContr, fu in enumerate(FUNC):
        ...
        for iterSbj, su in enumerate(SUBJ):
            ...
            PATH_OUT = os.path.join(STUDY_PATH, su, 'derivatives', 'func', 'AOM', 'vaso_analysis', CONDT[0], 'masks_maps', 'res_pt8', 'masks')
