...
#                 dpi=my_dpi)
# fig2.suptitle('2D Histogram for {}, {}'.format(MASK, tag))
...
for itSbj, si in enumerate(SUBJ):
    ...
    # Fig 1)
    fig1, axs1 = plt.subplots(nrows=1, ncols=1, figsize=(1920/my_dpi, 1920/my_dpi),
                dpi=my_dpi)
    ...
    for co in CONDT:
        ...
        for it, fu in enumerate(FUNC):
            ...
            PATH_ACT = os.path.join(STUDY_PATH, si, 'derivatives', 'func', 'AOM', 'vaso_analysis', co, 'masks_maps', 'res_pt2')

            PATH_LAY = os.path.join(STUDY_PATH, si, 'derivatives', 'anat', 'layers_4')

            # FILE1: Activation maps
            if tag == 'psc':
                FILE1 = os.path.join(PATH_ACT, '{}_{}_leftMT_Sphere16radius_5psc_scaled_4_unthreshold.nii.gz'.format(si, fu))
            else:
                FILE1 = os.path.join(PATH_ACT, '{}_{}_leftMT_Sphere16radius_tmaps_scaled_4.nii.gz'.format(si, fu))
            nii1 = nb.load(FILE1)
            ...
            # FILE2: Mask
            FILE2 = os.path.join(PATH_ACT, 'masks', "{}_leftMT_Sphere16radius_{}_mask_scaled_4.nii.gz".format(si, MASK[it]))
            nii2 = nb.load(FILE2)
            ...
            # Fig.2) Plotting: 2D histogram
            # im = axs2[itSbj, it].hist2d(metric[idx], y, bins=n_bins, cmap='magma', norm=mpl.colors.LogNorm())
            ...
            # Fig.1) Plotting layer profiles

            # Plotting: Layer Profiles
            axs1.plot(idx_layers, lay_act, linewidth=1, linestyle='-', marker='o', label='{}'.format(fu))
            axs1.set_ylim((0, 6))
            axs1.set_xlabel("Layers (0=white matter)")
            axs1.set_ylabel("Percent Signal Change")
            axs1.set_title("{} Layers Profiles".format(si))
            axs1.grid(True)
            axs1.legend()

            fig1.tight_layout()
            fig1.savefig(os.path.join(PATH_OUT,'{}_LayerProfiles_{}_mask_{}_{}'.format(si, ROI[0], MASK, tag)), bbox_inches='tight')
            fig1.show()
...
# fig2.tight_layout()
# fig2.savefig(os.path.join(PATH_OUT,'{}_2DHistogram_{}_mask_{}_black_magma_100_{}.svg'.format(SUBJ, ROI[0], MASK, tag)), bbox_inches='tight')
# fig2.savefig(os.path.join(PATH_OUT,'{}_2DHistogram_{}_mask_{}_black_magma_100_{}.jpeg'.format(SUBJ, ROI[0], MASK, tag)), bbox_inches='tight')
