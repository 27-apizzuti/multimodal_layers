...
for su in SUBJ:
    ...
    for roi in ROI_NAME:
        for fu in FUNC:
            ...
            # Find only voxels with positive t-values
            temp = np.sum(vox_neg, axis=1)
            idxAllPos = temp == 0
            ...
            # Export POSITIVE Voxels
            new_data = np.zeros(np.shape(vox_idx))
            new_data[vox_idx] =  idxAllPos.astype('uint8')
            outname = os.path.join(PATH_OUT, "{}_{}_{}_allPositiveTValues_voxels.nii.gz".format(su, roi, fu))
            img = nb.Nifti1Image(new_data, affine=nii1.affine)
            nb.save(img, outname)
