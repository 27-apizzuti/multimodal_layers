...
Created on Sat Jan 22 11:17:34 2022
    ...
    Scatterplot (sub-plot): Specificity vs Sensitivity (group result)
...
# MASK = ["BOLD_CV_allPositiveTValues", "VASO_CV_allPositiveTValues"]
# MASK = ""
...
for i, su in enumerate(SUBJ):
    ...
    for j, fu in enumerate(FUNC):
        ...
        print(np.max(vox_spec))
        axs[j].scatter(vox_sens,
                          vox_spec,
                          color='black',
                          alpha=0.2)
        axs[j].set_ylabel("Specificity (1-div)")
        axs[j].set_xlabel("Sensitivity (L2norm)")
        axs[j].set_ylim([0, 1])
        axs[j].set_xlim([0, 30])
        axs[j].grid("True")
        axs[j].set_title("{}".format(fu))

plt.suptitle("Voxel Characterization {} ({})\n\n".format(ROI_NAME, mask_name))
fig_filename = "Group_result_{}_Voxel_Characterization_{}.jpeg".format(ROI_NAME, CONDT[0], mask_name)
fig.tight_layout()
