...
ROIS = ['hMT', 'V1' ,'V2', 'V3']
HEMI = 'RH'
MAX = [3000, 2500, 2500, 3000] 
MAIN_PATH = 'D:\\AHEAD_v2\\derivatives\\T2star_in-vivo\\03_layers\\{}'.format(HEMI)
nii = nb.load(os.path.join(MAIN_PATH, "sub-01_part-mag_ME3DEPI_crop_ups2X_prepped_reg_echo-mean_{}_cut.nii.gz".format(HEMI)))
...
# // Create output path
PATH_OUT = os.path.join(MAIN_PATH, 'Figure_{}_wT2'.format(TAG))
...
for it_roi, roi in enumerate(ROIS):
    ...
    axs.set_title('{}, wT2*'.format(roi), fontsize=22, pad=20)
    ...
    fig1.savefig(os.path.join(PATH_OUT, '{}_2D-Hist_wT2star_ice'.format(roi)), bbox_inches='tight')

# Save numpy array: ROIS x CONTRAST x N_LAYERS
np.save(os.path.join(PATH_OUT, "ROI_{}_wT2star_vs_depth_{}.npy".format(HEMI, N_LAY)), Layer_database)
