...
MAIN_PATH = 'D:\\AHEAD_v2\\derivatives\\restingState'
nii = nb.load(os.path.join(MAIN_PATH, "sub-04_task-rest_acq-2depimb2_run-01_SCSTBL_3DMCTS_bvbabel_undist_fix_THPGLMF5c_sess-02_BBR_mean_res2X.nii.gz"))
...
# USER's ENTRIES
# Metric file
nii = nb.load(glob(os.path.join(MAIN_PATH, 'layers', "*_metric_equivol*"))[0])
metric = np.asarray(nii.dataobj)

# ROI file
nii = nb.load(glob(os.path.join(MAIN_PATH, "*sub-04_visual_areas_hMT_{}_bvbabel_rim_voronoi_noCapsule.nii.gz".format(HEMI)))[0])
