...
nii = nb.load(os.path.join(MAIN_PATH, "sub-01_part-mag_ME3DEPI_crop_ups2X_prepped_reg_S0_{}_cut.nii.gz".format(HEMI)))
...
# // Create output path
PATH_OUT = os.path.join(MAIN_PATH, 'Figure_{}_S0'.format(TAG))
...
for it_roi, roi in enumerate(ROIS):
    ...
    axs.set_ylabel("S0 [a.u.]", fontsize=20)
    axs.set_title('{}, S0'.format(roi), fontsize=22, pad=20)

    fig1.tight_layout()

    # Additional
    axs.set_ylim(2000, 6000)
    axs.tick_params(axis='x',labelsize=16)
    axs.tick_params(axis='y',labelsize=16)
    x = np.linspace(0, 1, nr_layers+1)
    axs.plot(x[1:], layer_bins[1:], linewidth=4, color= 'white', linestyle='-')
    fig1.savefig(os.path.join(PATH_OUT, '{}_2D-Hist_S0_ice'.format(roi)), bbox_inches='tight')
